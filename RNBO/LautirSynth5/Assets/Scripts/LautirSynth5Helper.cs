...
public class LautirSynth5Handle {
    ...
    [DllImport("LautirSynth5", CallingConvention = CallingConvention.StdCall)]
    ...
    internal LautirSynth5Handle(int key) {
        PluginKey = key;
    }
    ...
    private static LautirSynth5Handle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (LautirSynth5Handle)gch.Target;
    }
...
public class LautirSynth5Helper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static LautirSynth5Helper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("LautirSynth5HelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                LautirSynth5Helper helper = go.AddComponent<LautirSynth5Helper>();
                helper.Plugin = new LautirSynth5Handle(key);
                return helper;
            }
            return go.GetComponent<LautirSynth5Helper>();
        }
    }

    public LautirSynth5Handle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            LautirSynth5Handle.RegisterGlobalTransport(null);
        }
    }
}
