...
public class LautirSynth1Handle {
    ...
    [DllImport("LautirSynth1", CallingConvention = CallingConvention.StdCall)]
    ...
    internal LautirSynth1Handle(int key) {
        PluginKey = key;
    }
    ...
    private static LautirSynth1Handle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (LautirSynth1Handle)gch.Target;
    }
...
public class LautirSynth1Helper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static LautirSynth1Helper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("LautirSynth1HelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                LautirSynth1Helper helper = go.AddComponent<LautirSynth1Helper>();
                helper.Plugin = new LautirSynth1Handle(key);
                return helper;
            }
            return go.GetComponent<LautirSynth1Helper>();
        }
    }

    public LautirSynth1Handle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            LautirSynth1Handle.RegisterGlobalTransport(null);
        }
    }
}
