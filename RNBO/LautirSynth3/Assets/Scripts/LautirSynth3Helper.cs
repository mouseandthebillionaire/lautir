...
public class LautirSynth3Handle {
    ...
    [DllImport("LautirSynth3", CallingConvention = CallingConvention.StdCall)]
    ...
    internal LautirSynth3Handle(int key) {
        PluginKey = key;
    }
    ...
    private static LautirSynth3Handle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (LautirSynth3Handle)gch.Target;
    }
...
public class LautirSynth3Helper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static LautirSynth3Helper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("LautirSynth3HelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                LautirSynth3Helper helper = go.AddComponent<LautirSynth3Helper>();
                helper.Plugin = new LautirSynth3Handle(key);
                return helper;
            }
            return go.GetComponent<LautirSynth3Helper>();
        }
    }

    public LautirSynth3Handle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            LautirSynth3Handle.RegisterGlobalTransport(null);
        }
    }
}
