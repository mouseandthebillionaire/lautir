...
public class LautirSynth7Handle {
    ...
    [DllImport("LautirSynth7", CallingConvention = CallingConvention.StdCall)]
    ...
    internal LautirSynth7Handle(int key) {
        PluginKey = key;
    }
    ...
    private static LautirSynth7Handle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (LautirSynth7Handle)gch.Target;
    }
...
public class LautirSynth7Helper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static LautirSynth7Helper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("LautirSynth7HelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                LautirSynth7Helper helper = go.AddComponent<LautirSynth7Helper>();
                helper.Plugin = new LautirSynth7Handle(key);
                return helper;
            }
            return go.GetComponent<LautirSynth7Helper>();
        }
    }

    public LautirSynth7Handle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            LautirSynth7Handle.RegisterGlobalTransport(null);
        }
    }
}
