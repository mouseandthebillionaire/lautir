...
public class LautirSynth8Handle {
    ...
    private static void WebGLWarnOnce() { }

    [DllImport("LautirSynth8")]
    ...
    [DllImport("LautirSynth8", CallingConvention = CallingConvention.StdCall)]
...
#else
    private static bool s_webglWarned;
    private static void WebGLWarnOnce() {
        if (s_webglWarned) return;
        s_webglWarned = true;
        Debug.LogError("RNBO native plugin 'LautirSynth8' is not available on WebGL. WebGL builds cannot load native DLLs/bundles; use a JS/WASM bridge instead. The synth will be disabled.");
    }

    private static IntPtr RNBOInstanceCreate(out int key) { key = 0; return IntPtr.Zero; }
    private static void RNBOInstanceDestroy(IntPtr instance) { }
    private static void RNBOProcess(IntPtr instance, MillisecondTime now, float[] data, int channels, int nframes, int samplerate) { }
    private static MessageTag RNBOTag(IntPtr tagString) => 0;
    private static IntPtr RNBOGetDescription() => IntPtr.Zero;
    private static IntPtr RNBOGetPresets() => IntPtr.Zero;
    private static bool RNBOResolveTag(int key, MessageTag tag, out IntPtr tagStr) { tagStr = IntPtr.Zero; return false; }
    private static bool RNBOInstanceMapped(int key) => false;
    private static bool RNBOPoll(int key) => false;
    private static bool RNBOSetParamValue(int key, ParameterIndex index, ParameterValue value, MillisecondTime attime) => false;
    private static bool RNBOGetParamValue(int key, ParameterIndex index, out ParameterValue value) { value = 0; return false; }
    private static bool RNBOSetParamValueNormalized(int key, ParameterIndex index, ParameterValue value, MillisecondTime attime) => false;
    private static bool RNBOGetParamValueNormalized(int key, ParameterIndex index, out ParameterValue value) { value = 0; return false; }
    private static bool RNBOClearRegisteredCallbacks(int key) => false;
    private static bool RNBORegisterParameterEventCallback(int key, IntPtr callback, IntPtr handle) => false;
    private static bool RNBORegisterMessageEventCallback(int key, IntPtr callback, IntPtr handle) => false;
    private static bool RNBORegisterTransportEventCallback(int key, IntPtr callback, IntPtr handle) => false;
    private static bool RNBORegisterTempoEventCallback(int key, IntPtr callback, IntPtr handle) => false;
    private static bool RNBORegisterBeatTimeEventCallback(int key, IntPtr callback, IntPtr handle) => false;
    private static bool RNBORegisterTimeSignatureEventCallback(int key, IntPtr callback, IntPtr handle) => false;
    private static bool RNBORegisterGlobalTransportRequestCallback(IntPtr callback, IntPtr handle) => false;
    private static bool RNBORegisterTransportRequestCallback(int key, IntPtr callback, IntPtr handle) => false;
    private static bool RNBORegisterPresetCallback(int key, IntPtr callback, IntPtr handle) => false;
    private static bool RNBOSendMessageBang(int key, MessageTag tag, MillisecondTime atTime) => false;
    private static bool RNBOSendMessageNumber(int key, MessageTag tag, Float value, MillisecondTime atTime) => false;
    private static bool RNBOSendMessageList(int key, MessageTag tag, Float[] list, IntPtr listlen, MillisecondTime atTime) => false;
    private static bool RNBOSendMIDI(int key, byte[] data, IntPtr dataLen, MillisecondTime atTime) => false;
    private static bool RNBOSendTransportEvent(int key, bool running, MillisecondTime atTime) => false;
    private static bool RNBOSendTempoEvent(int key, Float bpm, MillisecondTime atTime) => false;
    private static bool RNBOSendBeatTimeEvent(int key, Float beattime, MillisecondTime atTime) => false;
    private static bool RNBOSendTimeSignatureEvent(int key, int numerator, int denominator, MillisecondTime atTime) => false;
    private static bool RNBOCopyLoadDataRef(int key, IntPtr id, float[] data, IntPtr datalen, IntPtr channels, IntPtr samplerate) => false;
    private static bool RNBOUnsafeLoadDataRef(int key, IntPtr id, float[] data, IntPtr datalen, IntPtr channels, IntPtr samplerate) => false;
    private static bool RNBOUnsafeLoadReadOnlyDataRef(int key, IntPtr id, float[] data, IntPtr datalen, IntPtr channels, IntPtr samplerate) => false;
    private static bool RNBOReleaseDataRef(int key, IntPtr id) => false;
    private static bool RNBOLoadPreset(int key, IntPtr payload) => false;
    private static bool RNBOGetPresetSync(int key, out IntPtr payloadPtr) { payloadPtr = IntPtr.Zero; return false; }
    private static void RNBOFreePreset(IntPtr payloadPtr) { }
    private static bool RNBOGetPreset(int key) => false;
    private static IntPtr RNBOReleaseHandles() => IntPtr.Zero;
#endif
    ...
    public static PatcherDescription PatcherDescription {
        get {
            if (patcherDescription == null) {
                IntPtr p = RNBOGetDescription();
                if (p == IntPtr.Zero) {
                    patcherDescription = new PatcherDescription();
                    return patcherDescription;
                }
                string descString = Marshal.PtrToStringAnsi(p);
    ...
    public static PresetEntry[] Presets {
        get {
            if (presets == null) {
                IntPtr p = RNBOGetPresets();
                if (p == IntPtr.Zero) {
                    presets = Array.Empty<PresetEntry>();
                    return presets;
                }
    ...
    public LautirSynth8Handle() {
        int key;
        ownedInstance = RNBOInstanceCreate(out key);
        if (ownedInstance == IntPtr.Zero) {
            IsAvailable = false;
            WebGLWarnOnce();
            PluginKey = 0;
            registered = true;
    ...
    internal LautirSynth8Handle(int key) {
        PluginKey = key;
    }
    ...
    private static LautirSynth8Handle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (LautirSynth8Handle)gch.Target;
    }
...
public class LautirSynth8Helper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static LautirSynth8Helper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("LautirSynth8HelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                LautirSynth8Helper helper = go.AddComponent<LautirSynth8Helper>();
                helper.Plugin = new LautirSynth8Handle(key);
                return helper;
            }
            return go.GetComponent<LautirSynth8Helper>();
        }
    }

    public LautirSynth8Handle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            LautirSynth8Handle.RegisterGlobalTransport(null);
        }
    }
}
