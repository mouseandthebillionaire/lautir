...
public class LautirSynth6Handle {
    ...
    [DllImport("LautirSynth6", CallingConvention = CallingConvention.StdCall)]
    ...
    internal LautirSynth6Handle(int key) {
        PluginKey = key;
    }
    ...
    private static LautirSynth6Handle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (LautirSynth6Handle)gch.Target;
    }
...
public class LautirSynth6Helper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static LautirSynth6Helper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("LautirSynth6HelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                LautirSynth6Helper helper = go.AddComponent<LautirSynth6Helper>();
                helper.Plugin = new LautirSynth6Handle(key);
                return helper;
            }
            return go.GetComponent<LautirSynth6Helper>();
        }
    }

    public LautirSynth6Handle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            LautirSynth6Handle.RegisterGlobalTransport(null);
        }
    }
}
