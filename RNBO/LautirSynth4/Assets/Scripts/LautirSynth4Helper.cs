...
public class LautirSynth4Handle {
    ...
    [DllImport("LautirSynth4", CallingConvention = CallingConvention.StdCall)]
    ...
    internal LautirSynth4Handle(int key) {
        PluginKey = key;
    }
    ...
    private static LautirSynth4Handle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (LautirSynth4Handle)gch.Target;
    }
...
public class LautirSynth4Helper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static LautirSynth4Helper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("LautirSynth4HelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                LautirSynth4Helper helper = go.AddComponent<LautirSynth4Helper>();
                helper.Plugin = new LautirSynth4Handle(key);
                return helper;
            }
            return go.GetComponent<LautirSynth4Helper>();
        }
    }

    public LautirSynth4Handle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            LautirSynth4Handle.RegisterGlobalTransport(null);
        }
    }
}
