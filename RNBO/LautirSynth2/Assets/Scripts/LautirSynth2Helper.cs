...
public class LautirSynth2Handle {
    ...
    [DllImport("LautirSynth2", CallingConvention = CallingConvention.StdCall)]
    ...
    internal LautirSynth2Handle(int key) {
        PluginKey = key;
    }
    ...
    private static LautirSynth2Handle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (LautirSynth2Handle)gch.Target;
    }
...
public class LautirSynth2Helper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static LautirSynth2Helper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("LautirSynth2HelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                LautirSynth2Helper helper = go.AddComponent<LautirSynth2Helper>();
                helper.Plugin = new LautirSynth2Handle(key);
                return helper;
            }
            return go.GetComponent<LautirSynth2Helper>();
        }
    }

    public LautirSynth2Handle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            LautirSynth2Handle.RegisterGlobalTransport(null);
        }
    }
}
