...
public class LautirSynthHandle {
    ...
    [DllImport("LautirSynth", CallingConvention = CallingConvention.StdCall)]
    ...
    internal LautirSynthHandle(int key) {
        PluginKey = key;
    }
    ...
    private static LautirSynthHandle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (LautirSynthHandle)gch.Target;
    }
...
public class LautirSynthHelper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static LautirSynthHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("LautirSynthHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                LautirSynthHelper helper = go.AddComponent<LautirSynthHelper>();
                helper.Plugin = new LautirSynthHandle(key);
                return helper;
            }
            return go.GetComponent<LautirSynthHelper>();
        }
    }

    public LautirSynthHandle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            LautirSynthHandle.RegisterGlobalTransport(null);
        }
    }
}
