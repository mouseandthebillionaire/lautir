...
public class arpSynthHandle {
    ...
    [DllImport("arpSynth", CallingConvention = CallingConvention.StdCall)]
    ...
    internal arpSynthHandle(int key) {
        PluginKey = key;
    }
    ...
    private static arpSynthHandle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (arpSynthHandle)gch.Target;
    }
...
public class arpSynthHelper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static arpSynthHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("arpSynthHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                arpSynthHelper helper = go.AddComponent<arpSynthHelper>();
                helper.Plugin = new arpSynthHandle(key);
                return helper;
            }
            return go.GetComponent<arpSynthHelper>();
        }
    }

    public arpSynthHandle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            arpSynthHandle.RegisterGlobalTransport(null);
        }
    }
}
