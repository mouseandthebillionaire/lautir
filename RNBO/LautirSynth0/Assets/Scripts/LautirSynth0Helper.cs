...
public class LautirSynth0Handle {
    ...
    [DllImport("LautirSynth0", CallingConvention = CallingConvention.StdCall)]
    ...
    internal LautirSynth0Handle(int key) {
        PluginKey = key;
    }
    ...
    private static LautirSynth0Handle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (LautirSynth0Handle)gch.Target;
    }
...
public class LautirSynth0Helper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static LautirSynth0Helper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("LautirSynth0HelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                LautirSynth0Helper helper = go.AddComponent<LautirSynth0Helper>();
                helper.Plugin = new LautirSynth0Handle(key);
                return helper;
            }
            return go.GetComponent<LautirSynth0Helper>();
        }
    }

    public LautirSynth0Handle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            LautirSynth0Handle.RegisterGlobalTransport(null);
        }
    }
}
